# nd_popup.py

import os
import time
import traceback
from datetime import datetime

import numpy as np
import vtk

from PyQt5 import QtWidgets, QtGui
from PyQt5.QtCore import Qt, QTimer

from unified_tf_canvas import UnifiedTFCanvas
from tf_canvas_widget import TFCanvasWidget
from gl_nd_volume_widget import GLNDVolumeWidget
from tf_texture_builder_debug import build_tf_texture_2d_debug

from matplotlib.backends.backend_qt5agg import FigureCanvasQTAgg as FigureCanvas
from matplotlib.figure import Figure


class NDFeaturePopup(QtWidgets.QMainWindow):
    def __init__(
        self,
        feat_x,
        feat_y,
        data_x,
        data_y,
        nd_manager,
        image_data,
        all_features,
        parent=None,
    ):
        super().__init__(parent)

        self.feat_x = feat_x
        self.feat_y = feat_y
        self.nd_manager = nd_manager
        self.image_data = image_data
        self.all_features = all_features

        self.current_widget = None
        self._render_update_pending = False

        # Adaptive rendering quality
        self.interactive_sampling_rate = 0.75
        self.final_sampling_rate = 2.0
        self.final_render_delay_ms = 2000

        self.final_render_timer = QTimer(self)
        self.final_render_timer.setSingleShot(True)
        self.final_render_timer.timeout.connect(self.render_final_quality)

        point_data = image_data.GetPointData()
        self.feature_names = []
        for i in range(point_data.GetNumberOfArrays()):
            name = point_data.GetArrayName(i)
            if name:
                self.feature_names.append(name)

        print(f"Actual features in point data: {self.feature_names}")

        self.setWindowTitle(f"nD Explorer: {feat_x} vs {feat_y}")
        self.setGeometry(200, 200, 1500, 850)
        self.setWindowFlags(Qt.Window | Qt.WindowCloseButtonHint | Qt.WindowMinimizeButtonHint)

        self.x_range = (float(np.min(data_x)), float(np.max(data_x)))
        self.y_range = (float(np.min(data_y)), float(np.max(data_y)))

        self.setup_ui(data_x, data_y)
        self.load_projected_widgets()
        self.update_render_view(interactive=False)
        self.interactive_timer = QTimer(self)
        self.interactive_timer.setSingleShot(True)
        self.interactive_timer.timeout.connect(self.finish_interactive_update)

    # ------------------------------------------------------------------
    # Data normalization
    # ------------------------------------------------------------------

    def normalize_to_255(self, data):
        data = np.asarray(data, dtype=np.float32)
        data_min = float(np.min(data))
        data_max = float(np.max(data))

        if data_max > data_min:
            return 255.0 * (data - data_min) / (data_max - data_min)

        return np.zeros_like(data, dtype=np.float32)

    # ------------------------------------------------------------------
    # UI setup
    # ------------------------------------------------------------------

    def setup_ui(self, data_x, data_y):
        central = QtWidgets.QWidget()
        self.setCentralWidget(central)
        main_layout = QtWidgets.QHBoxLayout(central)

        # ==============================================================
        # LEFT: 2D TF canvas
        # ==============================================================

        left_widget = QtWidgets.QWidget()
        left_layout = QtWidgets.QVBoxLayout(left_widget)

        title = QtWidgets.QLabel(f"<h2>{self.feat_x} vs {self.feat_y}</h2>")
        title.setAlignment(Qt.AlignCenter)
        left_layout.addWidget(title)

        info = (
            f"Raw: {self.feat_x}=[{self.x_range[0]:.3f}, {self.x_range[1]:.3f}] | "
            f"{self.feat_y}=[{self.y_range[0]:.3f}, {self.y_range[1]:.3f}]"
        )
        info_label = QtWidgets.QLabel(info)
        info_label.setStyleSheet("color: #666; font-size: 10px;")
        info_label.setAlignment(Qt.AlignCenter)
        left_layout.addWidget(info_label)

        norm_data_x = self.normalize_to_255(data_x)
        norm_data_y = self.normalize_to_255(data_y)

        self.tf_canvas = UnifiedTFCanvas(
            tf_type="2d",
            data=norm_data_x,
            gradient_data=norm_data_y,
            x_label=self.feat_x,
            y_label=self.feat_y,
        )

        self.tf_canvas.raw_intensity_range = self.x_range
        self.tf_canvas.raw_gradient_range = self.y_range
        self.tf_canvas.intensity_range = (0, 255)
        self.tf_canvas.gradient_range = (0, 255)

        self.tf_canvas.set_projection_features(self.feat_x, self.feat_y)
        self.tf_canvas.set_nd_callback(self.on_widget_moved)

        canvas_wrapper = TFCanvasWidget(self.tf_canvas, self, label="Reset View")
        left_layout.addWidget(canvas_wrapper)

        main_layout.addWidget(left_widget, 2)

        # ==============================================================
        # MIDDLE: controls
        # ==============================================================

        middle_widget = QtWidgets.QWidget()
        middle_layout = QtWidgets.QVBoxLayout(middle_widget)

        controls_title = QtWidgets.QLabel("<h3>Widget Controls</h3>")
        controls_title.setAlignment(Qt.AlignCenter)
        middle_layout.addWidget(controls_title)

        self.widget_list = QtWidgets.QListWidget()
        self.widget_list.itemSelectionChanged.connect(self.on_widget_selected)
        middle_layout.addWidget(self.widget_list)

        button_layout = QtWidgets.QHBoxLayout()

        add_btn = QtWidgets.QPushButton("Add Widget")
        add_btn.clicked.connect(self.add_widget)
        button_layout.addWidget(add_btn)

        delete_btn = QtWidgets.QPushButton("Delete Selected")
        delete_btn.clicked.connect(self.delete_selected_widget)
        button_layout.addWidget(delete_btn)

        clear_btn = QtWidgets.QPushButton("Clear All")
        clear_btn.clicked.connect(self.clear_all_widgets)
        button_layout.addWidget(clear_btn)

        middle_layout.addLayout(button_layout)

        self.param_group = QtWidgets.QGroupBox("Widget Parameters")
        self.param_layout = QtWidgets.QFormLayout()
        self.param_group.setLayout(self.param_layout)
        self.param_group.setVisible(False)
        middle_layout.addWidget(self.param_group)

        self.setup_render_controls(middle_layout)
        self.setup_widget_creation_controls(middle_layout)
        self.setup_texture_preview_controls(middle_layout)

        middle_layout.addStretch()
        main_layout.addWidget(middle_widget, 1)

        # ==============================================================
        # RIGHT: true 2D OpenGL renderer
        # ==============================================================

        right_widget = QtWidgets.QWidget()
        right_layout = QtWidgets.QVBoxLayout(right_widget)

        render_title = QtWidgets.QLabel(
            f"<h3>True 2D OpenGL Rendering: {self.feat_x} vs {self.feat_y}</h3>"
        )
        render_title.setAlignment(Qt.AlignCenter)
        right_layout.addWidget(render_title)

        self.tf_preview_label = QtWidgets.QLabel()
        self.tf_preview_label.setFixedHeight(180)
        self.tf_preview_label.setAlignment(Qt.AlignCenter)
        right_layout.addWidget(self.tf_preview_label)

        self.gl_widget = GLNDVolumeWidget()
        right_layout.addWidget(self.gl_widget, 1)

        projected_widgets = self.nd_manager.project_to_2d(self.feat_x, self.feat_y)

        self.gl_widget.set_volume_data(
            dims=self.image_data.GetDimensions(),
            all_features=self.all_features,
            feature_x=self.feat_x,
            feature_y=self.feat_y,
            widgets=projected_widgets,
        )

        self.gl_widget.sampling_rate = self.final_sampling_rate

        print(
            f"✅ True 2D OpenGL renderer initialized for "
            f"{self.feat_x} vs {self.feat_y}"
        )

        main_layout.addWidget(right_widget, 2)

    def setup_render_controls(self, parent_layout):
        group = QtWidgets.QGroupBox("OpenGL Render Controls")
        layout = QtWidgets.QFormLayout(group)

        self.vis_slider = QtWidgets.QSlider(Qt.Horizontal)
        self.vis_slider.setRange(10, 500)
        self.vis_slider.setValue(100)
        self.vis_slider.valueChanged.connect(self.on_visibility_changed)
        layout.addRow("Visibility Boost:", self.vis_slider)

        self.vis_value_label = QtWidgets.QLabel("1.00x")
        layout.addRow("", self.vis_value_label)

        self.op_slider = QtWidgets.QSlider(Qt.Horizontal)
        self.op_slider.setRange(25, 300)
        self.op_slider.setValue(100)
        self.op_slider.valueChanged.connect(self.on_opacity_scale_changed)
        layout.addRow("Opacity Correction:", self.op_slider)

        self.op_value_label = QtWidgets.QLabel("1.00x")
        layout.addRow("", self.op_value_label)

        self.interactive_sampling_spin = QtWidgets.QDoubleSpinBox()
        self.interactive_sampling_spin.setRange(0.25, 4.0)
        self.interactive_sampling_spin.setSingleStep(0.25)
        self.interactive_sampling_spin.setValue(self.interactive_sampling_rate)
        self.interactive_sampling_spin.valueChanged.connect(self.on_interactive_sampling_changed)
        layout.addRow("Moving Sampling:", self.interactive_sampling_spin)

        self.final_sampling_spin = QtWidgets.QDoubleSpinBox()
        self.final_sampling_spin.setRange(0.25, 8.0)
        self.final_sampling_spin.setSingleStep(0.25)
        self.final_sampling_spin.setValue(self.final_sampling_rate)
        self.final_sampling_spin.valueChanged.connect(self.on_final_sampling_changed)
        layout.addRow("Final Sampling:", self.final_sampling_spin)

        self.force_red_btn = QtWidgets.QPushButton("Toggle Red Debug")
        self.force_red_btn.clicked.connect(self.force_real_red)
        layout.addRow(self.force_red_btn)

        parent_layout.addWidget(group)

    def setup_widget_creation_controls(self, parent_layout):
        group = QtWidgets.QGroupBox("Create / Replace Widget")
        layout = QtWidgets.QVBoxLayout(group)

        type_layout = QtWidgets.QHBoxLayout()
        type_layout.addWidget(QtWidgets.QLabel("Shape:"))

        self.test_widget_type = QtWidgets.QComboBox()
        self.test_widget_type.addItems(
            ["Gaussian", "Rectangular", "Triangular", "Ellipsoid", "Diamond"]
        )
        self.test_widget_type.currentTextChanged.connect(self.on_test_shape_changed)
        type_layout.addWidget(self.test_widget_type)
        layout.addLayout(type_layout)

        pos_layout = QtWidgets.QHBoxLayout()

        pos_layout.addWidget(QtWidgets.QLabel("X:"))
        self.test_x = QtWidgets.QSpinBox()
        self.test_x.setRange(0, 255)
        self.test_x.setValue(128)
        self.test_x.valueChanged.connect(self.on_test_position_changed)
        pos_layout.addWidget(self.test_x)

        pos_layout.addWidget(QtWidgets.QLabel("Y:"))
        self.test_y = QtWidgets.QSpinBox()
        self.test_y.setRange(0, 255)
        self.test_y.setValue(128)
        self.test_y.valueChanged.connect(self.on_test_position_changed)
        pos_layout.addWidget(self.test_y)

        layout.addLayout(pos_layout)

        falloff_layout = QtWidgets.QHBoxLayout()
        falloff_layout.addWidget(QtWidgets.QLabel("Falloff:"))

        self.falloff_type = QtWidgets.QComboBox()
        self.falloff_type.addItems(["gaussian", "linear", "constant", "power2", "power3"])
        self.falloff_type.setCurrentText("gaussian")
        falloff_layout.addWidget(self.falloff_type)

        layout.addLayout(falloff_layout)

        browse_layout = QtWidgets.QHBoxLayout()

        prev_btn = QtWidgets.QPushButton("Previous")
        prev_btn.clicked.connect(self.prev_browse_widget)
        browse_layout.addWidget(prev_btn)

        self.browse_label = QtWidgets.QLabel("Gaussian")
        self.browse_label.setAlignment(Qt.AlignCenter)
        browse_layout.addWidget(self.browse_label)

        next_btn = QtWidgets.QPushButton("Next")
        next_btn.clicked.connect(self.next_browse_widget)
        browse_layout.addWidget(next_btn)

        layout.addLayout(browse_layout)

        apply_btn = QtWidgets.QPushButton("Apply to Selected")
        apply_btn.clicked.connect(self.apply_test_to_current_widget)
        layout.addWidget(apply_btn)

        self.fps_display = QtWidgets.QLabel("Ready")
        self.fps_display.setAlignment(Qt.AlignCenter)
        self.fps_display.setStyleSheet("font-weight: bold; color: #00aa00;")
        layout.addWidget(self.fps_display)

        screenshot_btn = QtWidgets.QPushButton("Take Screenshot")
        screenshot_btn.clicked.connect(self.take_screenshot)
        layout.addWidget(screenshot_btn)

        self.browse_shapes = ["Gaussian", "Rectangular", "Triangular", "Ellipsoid", "Diamond"]
        self.browse_index = 0

        parent_layout.addWidget(group)

    def setup_texture_preview_controls(self, parent_layout):
        separator = QtWidgets.QFrame()
        separator.setFrameShape(QtWidgets.QFrame.HLine)
        parent_layout.addWidget(separator)

        texture_label = QtWidgets.QLabel("<b>2D TF Texture Debug</b>")
        texture_label.setAlignment(Qt.AlignCenter)
        parent_layout.addWidget(texture_label)

        self.texture_display = TFTextureDisplay(self, width=4, height=4)
        parent_layout.addWidget(self.texture_display)

        refresh_btn = QtWidgets.QPushButton("Refresh Texture Preview")
        refresh_btn.clicked.connect(self.update_tf_texture_preview)
        parent_layout.addWidget(refresh_btn)

    # ------------------------------------------------------------------
    # Render controls
    # ------------------------------------------------------------------

    def on_visibility_changed(self, value):
        boost = value / 100.0
        self.vis_value_label.setText(f"{boost:.2f}x")

        if hasattr(self, "gl_widget") and self.gl_widget:
            self.gl_widget.visibility_boost = boost
            self.gl_widget.update()

    def on_opacity_scale_changed(self, value):
        scale = value / 100.0
        self.op_value_label.setText(f"{scale:.2f}x")

        if hasattr(self, "gl_widget") and self.gl_widget:
            self.gl_widget.opacity_scale = scale
            self.gl_widget.update()

    def on_interactive_sampling_changed(self, value):
        self.interactive_sampling_rate = float(value)

    def on_final_sampling_changed(self, value):
        self.final_sampling_rate = float(value)
        self.schedule_final_render()

    def force_real_red(self):
        if hasattr(self, "gl_widget") and self.gl_widget:
            self.gl_widget.force_red = not self.gl_widget.force_red
            print(f"[GL-ND] force_red={self.gl_widget.force_red}")
            self.gl_widget.update()

    # ------------------------------------------------------------------
    # Adaptive rendering
    # ------------------------------------------------------------------

    def render_interactive_quality(self):
        if hasattr(self, "gl_widget") and self.gl_widget:
            self.gl_widget.sampling_rate = self.interactive_sampling_rate
            self.gl_widget.update()

        self.final_render_timer.start(self.final_render_delay_ms)

    def render_final_quality(self):
        if hasattr(self, "gl_widget") and self.gl_widget:
            self.gl_widget.sampling_rate = self.final_sampling_rate
            self.gl_widget.update()

        if hasattr(self, "fps_display"):
            self.fps_display.setText(
                f"Final render: {self.final_sampling_rate:.2f}x sampling"
            )

    def schedule_final_render(self):
        self.render_interactive_quality()

    # ------------------------------------------------------------------
    # Widget loading / syncing
    # ------------------------------------------------------------------
    def request_interactive_render_update(self):
        if hasattr(self, "gl_widget"):
            self.gl_widget.set_interactive_mode(True)

        self.update_render_view(interactive=True)

        self.interactive_timer.start(1000)


    def finish_interactive_update(self):
        if hasattr(self, "gl_widget"):
            self.gl_widget.set_interactive_mode(False)

        self.update_render_view(interactive=False)

    def load_projected_widgets(self):
        self.tf_canvas.clear_widgets()

        projected = self.nd_manager.project_to_2d(self.feat_x, self.feat_y)

        for widget in projected:
            self.tf_canvas.add_widget(widget)

        print(f"Loaded {len(projected)} widgets into popup")
        self.update_widget_list()
        self.update_tf_texture_preview()

    def sync_to_nd(self):
        self.nd_manager.widgets.clear()

        for widget in self.tf_canvas.widgets:
            self.nd_manager.add_widget(widget)

        self.update_render_view(interactive=True)

    def load_from_nd(self):
        self.tf_canvas.clear_widgets()

        projected = self.nd_manager.project_to_2d(self.feat_x, self.feat_y)

        for widget in projected:
            self.tf_canvas.add_widget(widget)

        self.update_widget_list()
        self.update_render_view(interactive=True)

        print(f"Loaded {len(projected)} widgets from nD manager")

    def update_render_view(self, interactive=True):
        print(f"Updating render view for {self.feat_x} vs {self.feat_y}")

        if not hasattr(self, "gl_widget"):
            return

        widgets = self.nd_manager.project_to_2d(self.feat_x, self.feat_y)

        self.gl_widget.set_feature_pair(
            self.feat_x,
            self.feat_y,
            widgets,
        )

        # Preview only when NOT interactive (expensive UI update)
        if not interactive:
            self.update_tf_texture_preview()

        # Switch rendering quality
        if interactive:
            self.render_interactive_quality()
        else:
            self.render_final_quality()

    # ------------------------------------------------------------------
    # Widget movement
    # ------------------------------------------------------------------

    def on_widget_moved(self, widget_2d, feat_x, feat_y, new_x, new_y):
        print(
            f"Widget moved: ({new_x:.1f}, {new_y:.1f}) "
            f"for features ({feat_x}, {feat_y})"
        )

        widget_2d.center_intensity = new_x
        widget_2d.center_gradient = new_y

        if not hasattr(widget_2d, "nd_coords"):
            widget_2d.nd_coords = {}

        widget_2d.nd_coords[feat_x] = new_x
        widget_2d.nd_coords[feat_y] = new_y

        self.nd_manager.update_nd_position(widget_2d, new_x, new_y, feat_x, feat_y)

        self.update_widget_list()

        if hasattr(self, "test_x"):
            self.test_x.blockSignals(True)
            self.test_y.blockSignals(True)
            self.test_x.setValue(int(new_x))
            self.test_y.setValue(int(new_y))
            self.test_x.blockSignals(False)
            self.test_y.blockSignals(False)

        self.request_interactive_render_update()

    # ------------------------------------------------------------------
    # Widget list / selection
    # ------------------------------------------------------------------

    def update_widget_list(self):
        self.widget_list.clear()

        for i, widget in enumerate(self.tf_canvas.widgets):
            item = QtWidgets.QListWidgetItem(
                f"{i + 1}. {widget.widget_type.value} "
                f"(X:{widget.center_intensity:.0f}, Y:{widget.center_gradient:.0f})"
            )
            item.setData(Qt.UserRole, i)
            self.widget_list.addItem(item)

    def on_widget_selected(self):
        selected = self.widget_list.selectedItems()

        if not selected:
            self.param_group.setVisible(False)
            return

        idx = selected[0].data(Qt.UserRole)

        if idx < 0 or idx >= len(self.tf_canvas.widgets):
            self.param_group.setVisible(False)
            return

        self.current_widget = self.tf_canvas.widgets[idx]
        self.tf_canvas.active_widget = idx
        self.tf_canvas._draw()

        self.test_x.blockSignals(True)
        self.test_y.blockSignals(True)
        self.test_x.setValue(int(self.current_widget.center_intensity))
        self.test_y.setValue(int(self.current_widget.center_gradient))
        self.test_x.blockSignals(False)
        self.test_y.blockSignals(False)

        shape = self.identify_widget_shape(self.current_widget)
        self.test_widget_type.blockSignals(True)
        self.test_widget_type.setCurrentText(shape)
        self.test_widget_type.blockSignals(False)
        self.browse_label.setText(shape)

        self.update_parameter_controls()

    def identify_widget_shape(self, widget):
        if hasattr(widget, "intensity_radius"):
            return "Ellipsoid"

        if hasattr(widget, "direction"):
            return "Triangular"

        if hasattr(widget, "intensity_std"):
            return "Gaussian"

        if hasattr(widget, "intensity_width") and hasattr(widget, "gradient_height"):
            if widget.widget_type.value == "diamond":
                return "Diamond"
            return "Rectangular"

        return "Gaussian"

    # ------------------------------------------------------------------
    # Parameter controls
    # ------------------------------------------------------------------

    def update_parameter_controls(self):
        for i in reversed(range(self.param_layout.count())):
            item = self.param_layout.itemAt(i)
            if item and item.widget():
                item.widget().setParent(None)

        if self.current_widget is None:
            return

        widget = self.current_widget

        self.x_spin = QtWidgets.QDoubleSpinBox()
        self.x_spin.setRange(0, 255)
        self.x_spin.setSingleStep(1)
        self.x_spin.setValue(float(widget.center_intensity))
        self.x_spin.valueChanged.connect(self.on_x_changed)
        self.param_layout.addRow("X Position:", self.x_spin)

        self.y_spin = QtWidgets.QDoubleSpinBox()
        self.y_spin.setRange(0, 255)
        self.y_spin.setSingleStep(1)
        self.y_spin.setValue(float(widget.center_gradient))
        self.y_spin.valueChanged.connect(self.on_y_changed)
        self.param_layout.addRow("Y Position:", self.y_spin)

        self.opacity_spin = QtWidgets.QDoubleSpinBox()
        self.opacity_spin.setRange(0.0, 1.0)
        self.opacity_spin.setSingleStep(0.05)
        self.opacity_spin.setValue(float(widget.opacity))
        self.opacity_spin.valueChanged.connect(self.on_opacity_changed)
        self.param_layout.addRow("Opacity:", self.opacity_spin)

        self.blend_combo = QtWidgets.QComboBox()
        self.blend_combo.addItems(["max", "add", "multiply"])
        self.blend_combo.setCurrentText(getattr(widget, "blend_mode", "max"))
        self.blend_combo.currentTextChanged.connect(self.on_blend_mode_changed)
        self.param_layout.addRow("Blend Mode:", self.blend_combo)

        if hasattr(widget, "falloff_type"):
            self.falloff_combo = QtWidgets.QComboBox()
            self.falloff_combo.addItems(["gaussian", "linear", "constant", "power2", "power3"])
            self.falloff_combo.setCurrentText(widget.falloff_type)
            self.falloff_combo.currentTextChanged.connect(self.on_widget_falloff_changed)
            self.param_layout.addRow("Falloff Type:", self.falloff_combo)

        def add_size_control(label, attr, min_val=1, max_val=255, step=1):
            spin = QtWidgets.QDoubleSpinBox()
            spin.setRange(min_val, max_val)
            spin.setSingleStep(step)
            spin.setValue(float(getattr(widget, attr)))
            spin.valueChanged.connect(
                lambda value, a=attr: self.on_widget_param_changed(a, value)
            )
            self.param_layout.addRow(label, spin)

        if hasattr(widget, "intensity_std"):
            add_size_control("Intensity Std:", "intensity_std", 1, 150, 1)
            add_size_control("Gradient Std:", "gradient_std", 1, 150, 1)

        if hasattr(widget, "intensity_width"):
            add_size_control("Intensity Width:", "intensity_width", 1, 255, 1)

        if hasattr(widget, "gradient_height"):
            add_size_control("Gradient Height:", "gradient_height", 1, 255, 1)

        if hasattr(widget, "intensity_radius"):
            add_size_control("Intensity Radius:", "intensity_radius", 1, 150, 1)

        if hasattr(widget, "gradient_radius"):
            add_size_control("Gradient Radius:", "gradient_radius", 1, 150, 1)

        if hasattr(widget, "falloff"):
            add_size_control("Falloff:", "falloff", 0, 80, 1)

        if hasattr(widget, "falloff_power"):
            add_size_control("Falloff Power:", "falloff_power", 0.1, 5.0, 0.1)

        if hasattr(widget, "direction"):
            self.direction_combo = QtWidgets.QComboBox()
            self.direction_combo.addItems(["up", "down", "symmetric"])
            self.direction_combo.setCurrentText(widget.direction)
            self.direction_combo.currentTextChanged.connect(self.on_direction_changed)
            self.param_layout.addRow("Direction:", self.direction_combo)

        self.color_btn = QtWidgets.QPushButton("Change Color")
        self.color_btn.clicked.connect(self.change_color)
        self.param_layout.addRow("Color:", self.color_btn)
        self.update_color_button()

        self.param_group.setVisible(True)

    def on_widget_param_changed(self, attr, value):
        if self.current_widget is None:
            return

        setattr(self.current_widget, attr, value)

        print(f"Updated widget parameter: {attr} = {value}")

        self.tf_canvas._draw()
        self.sync_to_nd()
        self.update_widget_list()

    def on_blend_mode_changed(self, blend_mode):
        if self.current_widget is None:
            return

        self.current_widget.blend_mode = blend_mode

        print(f"Updated blend mode: {blend_mode}")

        self.tf_canvas._draw()
        self.sync_to_nd()

    def on_widget_falloff_changed(self, falloff_type):
        if self.current_widget is None:
            return

        self.current_widget.falloff_type = falloff_type

        print(f"Updated falloff type: {falloff_type}")

        self.tf_canvas._draw()
        self.sync_to_nd()

    def on_direction_changed(self, direction):
        if self.current_widget is None:
            return

        self.current_widget.direction = direction

        print(f"Updated widget direction: {direction}")

        self.tf_canvas._draw()
        self.sync_to_nd()

    def on_x_changed(self, value):
        if self.current_widget:
            self.current_widget.center_intensity = value
            self.tf_canvas._draw()
            self.sync_to_nd()

    def on_y_changed(self, value):
        if self.current_widget:
            self.current_widget.center_gradient = value
            self.tf_canvas._draw()
            self.sync_to_nd()

    def on_opacity_changed(self, value):
        if self.current_widget:
            self.current_widget.opacity = value
            self.tf_canvas._draw()
            self.sync_to_nd()

    def change_color(self):
        if not self.current_widget:
            return

        qcolor = QtWidgets.QColorDialog.getColor()

        if qcolor.isValid():
            self.current_widget.color = (
                qcolor.redF(),
                qcolor.greenF(),
                qcolor.blueF(),
            )

            self.update_color_button()
            self.tf_canvas._draw()
            self.sync_to_nd()

    def update_color_button(self):
        if hasattr(self, "color_btn") and self.current_widget:
            r, g, b = self.current_widget.color
            self.color_btn.setStyleSheet(
                f"background-color: rgb({int(r * 255)}, {int(g * 255)}, {int(b * 255)});"
            )

    # ------------------------------------------------------------------
    # Add / delete widgets
    # ------------------------------------------------------------------

    def add_widget(self):
        shape = self.test_widget_type.currentText()
        x = self.test_x.value()
        y = self.test_y.value()
        falloff = self.falloff_type.currentText()

        new_widget = self.create_test_widget(shape, x, y, falloff)
        new_widget.nd_ref = True

        self.nd_manager.add_widget(new_widget)
        self.tf_canvas.add_widget(new_widget)

        self.update_widget_list()
        self.update_render_view(interactive=True)

        print(
            f"Added {shape} widget at ({x}, {y}) "
            f"with nd_ref=True, falloff={falloff}"
        )

    def delete_selected_widget(self):
        if self.current_widget:
            self.tf_canvas.remove_widget(self.current_widget)
            self.current_widget = None
            self.param_group.setVisible(False)
            self.sync_to_nd()
            self.update_widget_list()

    def clear_all_widgets(self):
        self.tf_canvas.clear_widgets()
        self.current_widget = None
        self.param_group.setVisible(False)
        self.sync_to_nd()
        self.update_widget_list()

    # ------------------------------------------------------------------
    # Shape browse / replacement
    # ------------------------------------------------------------------

    def on_test_shape_changed(self, shape):
        self.fps_display.setText(f"Selected: {shape}")

    def on_test_position_changed(self):
        x = self.test_x.value()
        y = self.test_y.value()
        self.fps_display.setText(f"Position: ({x}, {y})")

    def apply_test_to_current_widget(self):
        if self.current_widget is None:
            QtWidgets.QMessageBox.warning(self, "No Widget", "Please select a widget first")
            return

        shape = self.test_widget_type.currentText()
        x = self.test_x.value()
        y = self.test_y.value()
        falloff = self.falloff_type.currentText()

        old_widget = self.current_widget

        new_widget = self.create_test_widget(shape, x, y, falloff)
        new_widget.color = old_widget.color
        new_widget.opacity = old_widget.opacity
        new_widget.blend_mode = getattr(old_widget, "blend_mode", "max")
        new_widget.nd_ref = True

        try:
            idx = self.tf_canvas.widgets.index(old_widget)
        except ValueError:
            return

        self.tf_canvas.widgets[idx] = new_widget

        try:
            nd_idx = self.nd_manager.widgets.index(old_widget)
            self.nd_manager.widgets[nd_idx] = new_widget
        except ValueError:
            pass

        self.current_widget = new_widget
        self.tf_canvas.active_widget = idx

        self.tf_canvas._draw()

        # Sync data only; do not use the normal interactive sync path here
        self.nd_manager.widgets.clear()
        for widget in self.tf_canvas.widgets:
            self.nd_manager.add_widget(widget)

        self.update_widget_list()
        self.update_parameter_controls()

        # Force high-quality rebuild after shape replacement
        self.update_render_view(interactive=False)

        self.fps_display.setText(f"Updated to {shape} at ({x}, {y})")

    def prev_browse_widget(self):
        self.browse_index = (self.browse_index - 1) % len(self.browse_shapes)
        self.replace_current_with_browse_shape()

    def next_browse_widget(self):
        self.browse_index = (self.browse_index + 1) % len(self.browse_shapes)
        self.replace_current_with_browse_shape()

    def replace_current_with_browse_shape(self):
        shape = self.browse_shapes[self.browse_index]
        self.browse_label.setText(shape)

        self.test_widget_type.blockSignals(True)
        self.test_widget_type.setCurrentText(shape)
        self.test_widget_type.blockSignals(False)

        if self.current_widget:
            self.apply_test_to_current_widget()

    def create_test_widget(self, shape, x, y, falloff_type=None):
        from widget_factory import WidgetFactory, WidgetType

        DEFAULT_WIDGET_SIZE = 50
        DEFAULT_GAUSSIAN_STD = DEFAULT_WIDGET_SIZE / 2.355  # FWHM ≈ 50
        DEFAULT_RADIUS = DEFAULT_WIDGET_SIZE / 2

        shape_map = {
            "Gaussian": WidgetType.GAUSSIAN,
            "Rectangular": WidgetType.RECTANGULAR,
            "Triangular": WidgetType.TRIANGULAR,
            "Ellipsoid": WidgetType.ELLIPSOID,
            "Diamond": WidgetType.DIAMOND,
        }

        if falloff_type is None:
            falloff_type = "gaussian"

        base_params = {
            "center_intensity": x,
            "center_gradient": y,
            "opacity": 0.8,
            "color": (1.0, 1.0, 1.0),
            "blend_mode": "max",
            "falloff_type": falloff_type,
        }

        if shape == 'Gaussian':
            params = {
                **base_params,
                'intensity_std': DEFAULT_GAUSSIAN_STD,
                'gradient_std': DEFAULT_GAUSSIAN_STD,
                'falloff_type': 'gaussian'
            }

        elif shape == 'Rectangular':
            params = {
                **base_params,
                'intensity_width': DEFAULT_WIDGET_SIZE,
                'gradient_height': DEFAULT_WIDGET_SIZE,
                'falloff': 0,
                'falloff_type': 'constant'
            }

        elif shape == 'Triangular':
            params = {
                **base_params,
                'intensity_width': DEFAULT_WIDGET_SIZE,
                'gradient_height': DEFAULT_WIDGET_SIZE,
                'direction': 'symmetric',
                'falloff_type': 'linear'
            }

        elif shape == 'Ellipsoid':
            params = {
                **base_params,
                'intensity_radius': DEFAULT_RADIUS,
                'gradient_radius': DEFAULT_RADIUS,
                'falloff_type': 'linear'
            }

        elif shape == 'Diamond':
            params = {
                **base_params,
                'intensity_width': DEFAULT_WIDGET_SIZE,
                'gradient_height': DEFAULT_WIDGET_SIZE,
                'falloff_type': 'linear'
            }
        else:
            params = base_params

        return WidgetFactory.create_widget(shape_map[shape], **params)

    # ------------------------------------------------------------------
    # Texture preview
    # ------------------------------------------------------------------

    def update_tf_texture_preview(self):
        widgets = self.nd_manager.project_to_2d(self.feat_x, self.feat_y)
        tf = build_tf_texture_2d_debug(widgets, size=256, verbose=False)

        if hasattr(self, "texture_display"):
            self.texture_display.update_texture(tf)

        if hasattr(self, "tf_preview_label"):
            img = np.clip(tf * 255.0, 0, 255).astype(np.uint8)

            alpha = img[:, :, 3:4]
            rgb = img[:, :, :3]

            # Make alpha visible without destroying RGB preview.
            preview = np.maximum(rgb, alpha)

            h, w, _ = preview.shape

            qimg = QtGui.QImage(
                preview.data,
                w,
                h,
                3 * w,
                QtGui.QImage.Format_RGB888,
            ).copy()

            pixmap = QtGui.QPixmap.fromImage(qimg)
            pixmap = pixmap.scaled(
                180,
                180,
                Qt.KeepAspectRatio,
                Qt.SmoothTransformation,
            )

            self.tf_preview_label.setPixmap(pixmap)

    # ------------------------------------------------------------------
    # Screenshot / close
    # ------------------------------------------------------------------

    def take_screenshot(self):
        screenshot_dir = "screenshots"
        os.makedirs(screenshot_dir, exist_ok=True)

        shape = self.test_widget_type.currentText()
        x = self.test_x.value()
        y = self.test_y.value()
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")

        filename = os.path.join(
            screenshot_dir,
            f"screenshot_{self.feat_x}_{self.feat_y}_{shape}_{x}_{y}_{timestamp}.png",
        )

        if hasattr(self, "gl_widget") and self.gl_widget:
            pixmap = self.gl_widget.grab()
            pixmap.save(filename)

            self.fps_display.setText(f"Screenshot: {filename}")
            print(f"Screenshot saved: {filename}")

    def closeEvent(self, event):
        if hasattr(self, "gl_widget"):
            self.gl_widget.close()

        event.accept()


class TFTextureDisplay(FigureCanvas):
    def __init__(self, parent=None, width=4, height=4, dpi=80):
        self.fig = Figure(figsize=(width, height), dpi=dpi)
        super().__init__(self.fig)
        self.setParent(parent)

        self.ax = self.fig.add_subplot(111)
        self.im = None
        self.setup_axes()

    def setup_axes(self):
        self.ax.set_xlim(0, 255)
        self.ax.set_ylim(0, 255)
        self.ax.set_xlabel("X feature")
        self.ax.set_ylabel("Y feature")
        self.ax.set_title("2D TF Texture")
        self.ax.grid(True, alpha=0.3)

    def update_texture(self, texture_rgba):
        if texture_rgba is None:
            return

        alpha = texture_rgba[:, :, 3]

        if self.im is None:
            self.im = self.ax.imshow(
                alpha,
                origin="lower",
                extent=(0, 255, 0, 255),
                aspect="auto",
                interpolation="nearest",
                cmap="hot",
            )
        else:
            self.im.set_data(alpha)

        self.im.set_clim(0.0, max(1e-6, float(np.max(alpha))))

        self.ax.set_title(f"2D TF Texture opacity, max={np.max(alpha):.2f}")
        self.draw()